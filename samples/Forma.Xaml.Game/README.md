...
`GameSession` owns deterministic simulation and collision. `GamePresenter` projects it into focused `INotifyPropertyChanged` view models. `GameHudView.xaml`, `GameSettingsView.xaml`, and `GameResultView.xaml` own the UI trees and typed bindings; `GameViewPresentation` attaches resource dictionaries, static/dynamic resource use, selector styles, and state/event storyboards through Forma's runtime APIs.
